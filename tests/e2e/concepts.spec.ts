...
test.describe('concept index', () => {
  ...
  test('layer filter narrows via shareable URL params', async ({ page }) => {
    ...
    await expect(page.locator('li[data-layer="core-mechanism"]:visible')).toHaveCount(10); // + evaluation (stub)
  ...
  test('status filter and combined filters work; form reflects the URL', async ({ page }) => {
    ...
    await expect(page.locator('li[data-status="complete"]:visible')).toHaveCount(21); // + memory, planning, mcp
  ...
  test('without JavaScript the full unfiltered list still renders', async ({ browser }) => {
    ...
    await expect(page.locator('li[data-layer]')).toHaveCount(24);
    await context.close();
  });
});
